...
const { createNewPlayer } = require('../../resource/player');
...
module.exports = {
	...
	async handleMessage(message) {
		...
		if (!checkAudioCommandPermission(message))
			return await message.reply({ content: "You don't have permission to do this command!", ephemeral: true });

		const mysql = message.client.mysql;
		if (!mysql) return await message.reply({ content: "Can not get your data from database, I'm sorry", ephemeral: true });
		...
		const queue = player.getQueue(message.guild) || player.createQueue(message.guild, {
			metadata: {
		...
		if (!playlistName) {
			...
			if (playlists.length == 0)
				return message.reply(`You don't have any playlist available to play, please save a playlist`);
			...
			if (tracks.length == 0) return message.reply(`No track name **${playlist.name}**`);
			playlist.urls = tracks.map(track => track.url);
		}

		if (queue.nowPlaying()) {
			message.reply(`❤ | Add playlist **${playlist.name}**`)
		} else message.reply(`❤ | Play playlist **${playlist.name}**`);
